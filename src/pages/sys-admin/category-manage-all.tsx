...
import { ControlledTypography, UncontrolledCard, ControlledGrid, ControlledChip } from "@/components";
...
export type categoryManagementCreation = z.infer<typeof categoryManagementBaseSchema>
const CategoryForm = () => {
    const [categoryType, setCategoryType] = useState<Array<{
        label: string
        value: string
        name: string
    }>>([
    ...
    const {
        control,
    } = useFormContext<categoryManagementCreation>()
...
const CategoryManageAll: React.FC = () => {
    const [categoryManageAtom, setCategoryManageAtom] = useAtom(categoryManagementAtom)
    ...
    const CreationProductCategory = useApiCallBack(async (api, args : {label: string, value: string, type: string}) => await api.mdr.CreateProductCategory(args))
    const GetAllProductCategory = useApiCallBack(api => api.mdr.FetchAllCategories())
    const {
        handleOnToast
    ...
    const handleShowPopOver = (event: React.MouseEvent<HTMLButtonElement>, id: any) => {
        setCategoryId(id)
    ...
    const columns: any[] = [
        ...
        { field : 'label', headerName: 'Category Name', sortable : false, width: 160},
        {
            field: 'type',
