...
import {
    AccountOwnershipDetailsForm, CredentialsOwnershipDetailsForm,
    Completed
} from './forms'
            ...
            renderCell : (params: any) => (
                <>
                    <div style={{
                       ...
                       color='primary'
                       size='small'
                       onClick={() => handleClickProjectTable({id : params.row.id, propstype: 'edit',
                    firstname: params.row.firstname, middlename: params.row.middlename,
                        ...
                        (
                            ...
                            <>
                                <ControlledButton 
                                    ...
                                    color='success'
                                    size='small'
                                    onClick={() => handleClickProjectTable({id : params.row.id, propstype: 'verify'})}
                                />
                            </>
                        )
                       } &nbsp;
                        ...
                        (
                            <>
                                 <ControlledButton 
                                    ...
                                    color='success'
                                    size='small'
                                    onClick={() => handleClickProjectTable({id : params.row.id, propstype: 'unlock'})}
                                />
                            </>
                        ) : (
                            <>
        ...
        switch(propstype){
            case "edit":
                const values = {
                    ...
                    lastName: lastname,
                    hasNoMiddleName: middlename == null || middlename == "" ? false : true
                }
    ...
    const OnModalClose = () => {
        switch(identifier){
            case "unlock":
                ...
                UpdateUsersVerifiedStatusExecutioner.execute(argsUnlock).then((repository: any) => {
                ...
                case "lock":
                    ...
                    UpdateUsersVerifiedStatusExecutioner.execute(argsLock).then((repository: any) => {
                    const { data } : any = repository;
                    if(data === 200) {
                        handleOnToast(
                            "Successfully Lock.",
                    ...
                    case "verify":
                        ...
                        UpdateUsersVerifiedStatusExecutioner.execute(argsVerify).then((repository: any) => {
                    const { data } : any = repository;
                    if(data === 200) {
                        handleOnToast(
            ...
            >
                {
                    valueChange == 0 ? (
                        <>
                            <OnboardingStepper steps={['Basic Information', 'Credentials Information', 'Completed']} sx={{mt: 3}} activeStep={activeStep} />
                            <Box mt={2} width="100%">
                                <ActiveForm />
                            </Box>
                        </>
