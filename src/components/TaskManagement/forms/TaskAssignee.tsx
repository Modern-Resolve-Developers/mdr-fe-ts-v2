...
const TaskAssigneeForm = () => {
    const FetchUsersBasedType = useApiCallBack((api, usertype: any) => api.mdr.fetchUsersBasedType(usertype))
    const [assignee, setAssignee] = useState<Array<{name : string, label: string, value: string}>>([])
    const [reporter, setReporter] = useState<Array<{name : string, label: string, value: string}>>([
        {
            name: 'JM',
            label: 'JM',
            value: 'jm'
    ...
    const [department, setDepartment] = useState<Array<{name : string, label: string, value: string}>>([
        {
            name: 'SQE',
            label: 'SQE',
            value: 'sqe'
    ...
    const {
        control
    } = useFormContext<TaskAssigneeCreation>()
    useEffect(() => {
        FetchUsersBasedType.execute('2')
        .then((repo: any) => {
            const { data }: any = repo;
            setAssignee(
                [
...
export const TaskAssigneeDetailsForm = () => {
    const [taskAssigneeDetailsAtom, setTaskAssigneeDetailsAtom] = useAtom(taskAssigneeAtom)
    ...
    const handleContinue = () => {
        handleSubmit(
            (values) => {
                ...
                setOpen(!open)
                CreateTaskApi.execute(create_task_request)
                .then((repository: any) => {
